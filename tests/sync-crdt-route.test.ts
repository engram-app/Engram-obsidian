...
// ---------------------------------------------------------------------------
// reconcileColdStart: resumed KNOWN note adopts the server lineage instead of
// seeding a client lineage (#846 / e2e test_82). On resume a history-less doc
// for a note the server already holds must NOT be seeded from disk — seeding
// mints a second (client) lineage that Yjs unions with the server's on STEP2 →
// doubled body → the resumed device pushes the double and clobbers the peer's
// real edit. The discriminator is `serverKnown` (crdtHead, path-keyed, map-free
// so a resume-empty noteIdMap can't misroute it) + a history-less doc.
// ---------------------------------------------------------------------------

describe("reconcileColdStart adopt-vs-seed discriminator (#846/test_82)", () => {
	test("history-less + server-KNOWN → adopts server lineage, does NOT seed a client lineage", async () => {
		const applyLocalEdit = mock(async () => {});
		const adopt = mock(async () => "adopted");
		const hasHistory = mock(async () => false); // resume: doc has no baseline lineage
		await reconcileColdStart(
			{ path: "n.md", noteId: "id-n", diskContent: "original", serverKnown: true },
			{
				applyLocalEdit,
				getText: mock(async () => ""),
		...
		expect(adopt).toHaveBeenCalledWith("n.md", "id-n");
		// The fix: NO client-lineage seed → no #846 doubling on the STEP2 adopt.
		expect(applyLocalEdit).not.toHaveBeenCalled();
	});

	test("history-less + server-UNKNOWN → seeds (genuine offline-created note, never adopt)", async () => {
		// A locally-created note the server has never seen has no crdtHead. Routing
		// it to adopt would getUpdates a nonexistent note → never converges. It must
		// still seed its offline content.
		const applyLocalEdit = mock(async () => {});
		const adopt = mock(async () => "adopted");
		const hasHistory = mock(async () => false);
		await reconcileColdStart(
			{ path: "n.md", noteId: "id-n", diskContent: "brand new offline", serverKnown: false },
			{
				applyLocalEdit,
				getText: mock(async () => ""),
				projectedText: mock(async () => ""),
		...
		expect(applyLocalEdit).toHaveBeenCalledWith("id-n", "brand new offline");
	});

	test("history-FULL + server-known → diffs onto the existing baseline (NOT re-adopt)", async () => {
		// A doc that already carries its baseline lineage is site B (safe): the
		// disk edit diffs onto the existing lineage, no second lineage is minted.
		// Re-adopting would be wasteful and could revert an un-flushed local edit.
		const applyLocalEdit = mock(async () => {});
		const adopt = mock(async () => "adopted");
		const hasHistory = mock(async () => true);
		await reconcileColdStart(
			{ path: "n.md", noteId: "id-n", diskContent: "edited", serverKnown: true },
			{
				applyLocalEdit,
				getText: mock(async () => "old"),
				projectedText: mock(async () => "old"),
		...
		expect(adopt).not.toHaveBeenCalled();
		expect(applyLocalEdit).toHaveBeenCalledWith("id-n", "edited");
	});

	test("adopt path does NOT fire the disk-seed enroll (adopt is its own handshake)", async () => {
		const adopt = mock(async () => "adopted");
		const enroll = mock(() => {});
		await reconcileColdStart(
			{ path: "n.md", noteId: "id-n", diskContent: "original", serverKnown: true },
