# Provider-model CRDT cutover — status + finish map

Goal: abandon the bespoke CRDT machine, rebuild the client sync on a persistent
y-protocols provider, full swap.
...
| `612944a` | **`NoteProvider`** — the provider sync core. Persistent doc, `syncStep1` state-vector diff on reconnect (never a full re-push), `readSyncMessage` convergence (NO text-verify), buffer+flush offline edits. |
...
6 new TDD tests (2-device in-memory transport: converge / no-doubling-on-reconnect / adopt-first). Full unit suite **2235 green**.

### KEY finding (read before finishing)
Our `channel.ts` already mirrors the standard exchange (`startSync`=writeSyncStep1,
...
#846 doubling, the stale-snapshot guard that prevents deleting remote ops) — a
design where the doc IS the source of truth needs none of it; ours mirror
...
4. **`setConnected` lifecycle wired (the load-bearing fix).** The provider
...
6. **Validate.** Unit suite **2171 pass / 0 fail**, src build + biome clean. ⏳
   ...
   E2E-only) + a manual file-switch test. Do NOT merge without E2E green.

## Parallel blocker (independent of the rewrite)
The plugin **reloads every ~15-30s** in one env (proven: fresh-instance
signature on every socket rebuild; NOT Hot Reload). The provider model *survives*
