...
var EngramApi = class _EngramApi {
  ...
  async reportIssue(description, appVersion) {
...
var import_obsidian24 = require("obsidian"), ReportIssueState = class {
  constructor() {
    this.description = "";
    this.view = "editing";
    this.errorText = "";
  ...
  async submit(send) {
    if (this.canSubmit()) {
      ...
      try {
        await send(this.description.trim()), this.view = "success";
      } catch (e) {
        this.view = "error", this.errorText = "Couldn't send your report. Try again later.";
      }
    }
  }
}, ReportIssueModal = class extends import_obsidian24.Modal {
  constructor(app, api, appVersion) {
    super(app);
    this.api = api;
    this.appVersion = appVersion;
    this.state = new ReportIssueState();
  }
  onOpen() {
    this.contentEl.addClass("engram-report-issue-modal"), this.render();
  }
  onClose() {
  ...
  render() {
    let { contentEl } = this;
    if (contentEl.empty(), this.state.view === "success") {
      contentEl.createEl("p", { text: "Thanks. Your report was sent." }), contentEl.createEl("button", { text: "Close", cls: "mod-cta" }).addEventListener("click", () => this.close());
      return;
    }
    contentEl.createEl("h3", { text: "Report an issue" }), contentEl.createEl("p", {
    ...
    textarea.value = this.state.description, textarea.addEventListener("input", () => this.state.setDescription(textarea.value)), this.state.view === "error" && contentEl.createEl("p", {
      text: this.state.errorText,
      cls: "engram-report-issue-error"
    });
    let submit = contentEl.createDiv({ cls: "engram-report-issue-footer" }).createEl("button", {
      text: this.state.view === "submitting" ? "Sending..." : "Send report",
      cls: "mod-cta"
    });
    submit.disabled = this.state.view === "submitting", submit.addEventListener("click", () => {
      this.doSubmit();
    }), textarea.focus();
  }
  async doSubmit() {
    await this.state.submit(
      (description) => this.api.reportIssue(description, this.appVersion)
    ), this.render();
  }
};
  ...
  async onload() {
    ...
    saved != null && saved.lastSync && this.syncEngine.setLastSync(saved.lastSync), saved != null && saved.syncCursor && this.syncEngine.setSyncCursor(saved.syncCursor), (_a = saved == null ? void 0 : saved.offlineQueue) != null && _a.length && this.syncEngine.queue.load(saved.offlineQueue), (saved == null ? void 0 : saved.syncStateVaultId) !== void 0 && this.syncEngine.setSyncStateVaultId(saved.syncStateVaultId), saved != null && saved.syncState ? this.syncEngine.importSyncState(saved.syncState) : saved != null && saved.syncedHashes && (this.syncEngine.importHashes(saved.syncedHashes), devLog().log("lifecycle", "Migrated legacy syncedHashes \u2192 syncState")), this.syncEngine.issues.hydrate(saved == null ? void 0 : saved.syncIssues), this.syncEngine.ignoredFiles.hydrate(saved == null ? void 0 : saved.ignoredFiles), this.settings.planState && this.syncEngine.hydratePlanState(this.settings.planState), this.settingTab = new EngramSyncSettingTab(this.app, this), this.addSettingTab(this.settingTab), this.registerEvent(
      ...
      this.app.vault.on("delete", (file) => {
        file instanceof import_obsidian26.TFolder ? this.syncEngine.handleFolderDelete(file) : this.syncEngine.handleDelete(file);
      })
    ), this.registerEvent(
  ...
  connectChannel(attempt = 0, epoch = this.channelEpoch) {
    ...
    rlog().info(
      ...
      if (channel.setAuthProbe(() => this.api.getMe()), channel.onEvent = (event) => {
        ...
        var _a2;
        new import_obsidian26.Notice("Engram: This vault has been deleted on the server."), rlog().info("lifecycle", "Vault deleted on server \u2014 clearing vaultId"), this.settings.vaultId = null, this.api.setVaultId(null), this.savePluginData(this.syncEngine.getLastSync()), (_a2 = this.noteStream) == null || _a2.disconnect();
        ...
        this.crdtManager = new CrdtManager({
          ...
          isUnchangedSynced: (noteId, content) => {
            let path = this.noteIdMap.pathForId(noteId);
            return path ? this.syncEngine.isUnchangedSynced(path, content) : !1;
          },
          onPersistError: (noteId, err) => {
            var _a2;
            let path = (_a2 = this.noteIdMap.pathForId(noteId)) != null ? _a2 : noteId;
            rlog().warn(
              "crdt",
          ...
          startSync: (noteId) => {
            var _a2, _b2;
            return (_b2 = (_a2 = this.crdtChannel) == null ? void 0 : _a2.startSync(noteId)) != null ? _b2 : Promise.resolve();
          },
          resetSync: (noteId) => {
            var _a2;
            return (_a2 = this.crdtChannel) == null ? void 0 : _a2.resetSync(noteId);
          },
          ...
          onAfterEnroll: async (noteId) => {
            var _a2;
            await ((_a2 = this.crdtManager) == null ? void 0 : _a2.flattenIfBloated(noteId));
          }
          ...
          ), this.crdtEverJoined = !1, this.syncEngine.setCrdtManager(null), (_a2 = this.crdtManager) == null || _a2.clearSynced(), (_b2 = this.crdtEnrollment) == null || _b2.resetAll(), reason === "crdt_proto_too_old" && (this.crdtProtoTooOldNoticeShown || (this.crdtProtoTooOldNoticeShown = !0, new import_obsidian26.Notice(
